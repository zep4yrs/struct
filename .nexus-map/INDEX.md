...
**StructVis（位于 `structvis/`）**：数据结构与数据库自学工具，纯前端 SvelteKit 静态站（adapter-static，输出根 `docs/`，GitHub Pages `/struct` 部署）。核心价值：把算法/SQL/SQL 建表/索引动画变成可步进、可试错、可练习、可投影讲授的交互演示，对齐李春葆《数据结构教程》与杨宏霞《数据库技术及应用（MySQL）》。状态：**v0.2/v0.3 已完成**，**v1.0 图专题已完成**，**v1.0 补齐三批已完成（二分查找/KMP/BST/哈夫曼树/哈希表已上线）**，**v0.5 数据库专题收尾已完成（高级查询/概述/事务/用户权限，杨宏霞教材范围全覆盖）**。
...
- `routes/` — **29 个功能页面，无占位页**：`/`、`/ds`、`/db`、ds 全 19 页（5 排序+树+链表+栈队列+图遍历+最小生成树+最短路径+拓扑排序+关键路径+二分查找+KMP+二叉搜索树+哈夫曼树+哈希表+图的存储）、db 七页（er/index/normalize/sql/tables/update/advanced-query + overview/transaction/users 三概念/演示页）、`/progress`、`/settings`（设置）；`+layout.ts` prerender=true、trailingSlash='always'
...
- **测试**：364 个测试 / 36 spec 文件（`npm run test`，server node + client jsdom 双项目，组件测试用 @testing-library/svelte），`requireAssertions` 强制断言；`npm run check`（svelte-check 0 errors）、`npm run build` 自建 + 部署产物进仓库根 `docs/` 已验证；E2E 23/23（`npm run test:e2e`，水合竞态/时序坑位见 test_coverage.md 备忘）
