...
- 目录页：`/` 首页（卡片导航）、`/ds`、`/db`
- ds 播放器页：quick-sort、bubble-sort、insertion-sort、merge-sort、selection-sort、binary-tree、linear-list、stack-queue、graph-traversal、mst、shortest-path（共 11）
...
- 图专题后续增量 — 拓扑排序、关键路径（AOE）、图的存储（邻接矩阵/邻接表）；复用 `graph` 渲染器与契约；`settings` 无 `/settings` 路由（TopBar 占用齿轮位）