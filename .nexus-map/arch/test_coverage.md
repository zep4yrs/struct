...
- 引擎 spec 与引擎源码同目录（`*.spec.ts`），共 5 个 spec 文件 / 19 个测试
...
| `engines/algorithm/stackqueue/StackQueueEngine.spec.ts` | 7 | StackQueueEngine | push/pop/enqueue/dequeue 步骤生成、renderType 随结构切换、出栈目标与栈顶一致性、空栈下溢、练习题随结构切换 |
...
- **渲染器无测试**：5 个 Canvas 渲染器（布局/高亮绘制）无测试，也无 visual regression 基建；Canvas 在 node 环境不可测，属计划外
