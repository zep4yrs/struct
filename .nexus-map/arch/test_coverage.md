...
> provenance: 文件系统扫描 + vitest 配置阅读 + 实际运行（150/150 通过）；v1.0 图专题收尾已运行 `npm run test`

## 测试配置

- 运行器：Vitest v4（`test:unit` 脚本），node 环境，`requireAssertions: true`（强制每条测试必须有断言）
- 命令：`npm run test`（CI 一键）、`npm run test:unit`（watch）
- 引擎 spec 与引擎源码同目录（`*.spec.ts`），共 **20 个 spec 文件 / 150 个测试**
...
| `engines/algorithm/demoScript.spec.ts` | 2 | **讲授剧本（v0.3 新增）** | 11 引擎 demoScript 覆盖实际产生的全部步骤类型（default 除外）；SelectEngine presenterNote 优先于类型旁白 |
...
- **实际运行验证**：`npm run check`（svelte-check 0 errors）、`npm run test`（150/150）、`npm run build`（adapter-static 全量预渲染到根 `docs/`）本轮均通过；lint 仅对改动文件格式化，`prettier --check` 全量仍有历史欠账（InsertionSortEngine.spec.ts、StackQueueEngine.ts）