...
- **目录页章节面板化**：87 课题按章节切进淡色礁盘（数据结构暖调 / 数据库学术蓝调），组头带序号+课题数+组均掌握度条；分段 tab 与章节锚点合并同排；课题行加章节内编号（教材条目感）
- **技能图谱章节群岛**：13 章节各一块礁盘容器（课程系实线暖框 / 实验区虚线蓝框 + 组名 + 关数 + 组掌握度），盘内 8 列换行；节点内嵌掌握度微条；轨道过滤器联动礁盘淡化
- **实验室定位分层（ROADMAP 批次五 A+C）**：竞速=「观察场」、SQL 工作台=「动手场」段首定位；目录工具区补 SQL 工作台第三入口 + 「先看竞速建立直觉 → 再去工作台动手」动线提示
- 测试规模：495 单测 + 71 e2e
