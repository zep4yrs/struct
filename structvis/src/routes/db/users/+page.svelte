<script lang="ts">
	...
	let chosen = $state<(number | null)[]>(QUIZ.map(() => null));
	let finished = $state(false);

	const correctCount = $derived(answered.filter((i) => chosen[i] === QUIZ[i].correct).length);

	function pick(qi: number, oi: number) {
		if (answered.includes(qi)) return;
		chosen[qi] = oi;
		answered = [...answered, qi];
		if (answered.length === QUIZ.length) finished = true;
	}
</script>
...
<div class="page">
	...
	<section class="quiz-section">
		<div class="section-label">
			...
			<span class="section-name">安全练习</span>
		</div>
		...
		{#if finished}
			<div class="quiz-finished" aria-live="polite">
				...
				全部作答完毕：{correctCount} / {QUIZ.length} 正确
